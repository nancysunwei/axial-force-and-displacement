...
st.title("Axially Loaded Bars Analysis Module")
...
with st.sidebar:
    ...
    # 元素数量
    n_elem = st.number_input("Element Count):", min_value=1, max_value=20, value=st.session_state['n_elem'], step=1)
    ...
    u_area = st.selectbox("Area", ["mm2", "m2", "in2", "ft2"], index=0 if is_si else 2)
    u_mod = st.selectbox("Modulus", ["GPa", "MPa", "Pa", "ksi", "psi"], index=0 if is_si else 4)
    u_force = st.selectbox("Force", ["kN", "N", "kips", "lb"], index=0 if is_si else 3)
    u_disp = st.selectbox("Displacements", ["mm", "m", "in", "ft"], index=0 if is_si else 2)
...
if st.button("🚀 Compute & Plot", type="primary"):
    try:
        ...
        res_col1.metric(label=f"Max Force @ Elem {max_f_idx+1}", value=f"{max_f_val:.2f} {u_force}")
        res_col2.metric(label=f"Max Disp @ Node {max_d_idx+1}", value=f"{max_d_val:.4f} {u_disp}")
        ...
        ax1.set_title("Axial Force Diagram", fontsize=12, fontweight='bold')
        ...
        ax2.set_title("Displacement Diagram", fontsize=12, fontweight='bold')
